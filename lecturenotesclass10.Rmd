...
two_roll_results <- tibble(
  roll_1 = sample(dice_roll, size = 10000, replace = TRUE),
...
two_roll_results <- two_roll_results %>% 
  mutate(event1 = (roll_1 ^ 2) == roll_2, event2 = abs(roll_2 - roll_1) == 4)
...
```{r summarize_rolls}
two_roll_results %>% 
  summarise(event1_prob = mean(event1), event2_prob = mean(event2))
```
